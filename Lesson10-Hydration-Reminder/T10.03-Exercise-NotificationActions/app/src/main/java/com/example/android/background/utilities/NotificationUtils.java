...
public class NotificationUtils {
    ...
    private static final int WATER_DISMISS_NOTIFICATION_ID = 3418;
    private static final int WATER_INCREMENT_ID = 3419;

    //  complete (1) Create a method to clear all notifications
    public static void clearWaterNotification(Context context) {
        NotificationManager nm = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
        nm.cancel(WATER_REMINDER_NOTIFICATION_ID);
    }
    ...
    //      complete (8) Create a PendingIntent from the intent to launch WaterReminderIntentService
        PendingIntent pendingIntent = PendingIntent.getService(context,WATER_DISMISS_NOTIFICATION_ID, intent, PendingIntent.FLAG_UPDATE_CURRENT);
    //      complete (9) Create an Action for the user to ignore the notification (and dismiss it)
    //      complete (10) Return the action
        return new NotificationCompat.Action(R.drawable.ic_cancel_black_24px,"Dismiss", pendingIntent);
    }
    ...
    private static NotificationCompat.Action drinkWaterAction(Context context) {
        ...
        PendingIntent pendingIntent = PendingIntent.getService(context,WATER_INCREMENT_ID,intent,PendingIntent.FLAG_UPDATE_CURRENT);
        //      complete (15) Create an Action for the user to tell us they've had a glass of water
        //      complete (16) Return the action
        return new NotificationCompat.Action(R.drawable.ic_local_drink_black_24px,"Already did",pendingIntent);
    }
